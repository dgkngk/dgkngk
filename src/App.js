    ...
    if (this.state.selection !== "nothing") {
      return (
        <div>
          ...
          <br></br>
          <p style={{ color: "white" }}>
            Currently Displaying: {this.state.selection}
          </p>
...
class Clock extends React.Component {
  ...
  render() {
    return <h>{this.state.date.toLocaleTimeString()}</h>;
  }
}
...
function ApiCall(props) {
  ...
  return (
    <div>
      ...
      <table
        style={{ fontSize: "18px" }}
        className="table table-dark table-hover table-striped table-bordered border-success rounded-3"
      >
        <thead>
          <tr>
...
function ListElement(props) {
  ...
  return Object.keys(obj).map((d, key) => <th key={kk}>{obj[d]}</th>);
}
